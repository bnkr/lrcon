...
#
# [Bunker's note: this will work with Qt3, but you need Qt4's moc (and perhaps
# the other programs too) because the format of the arguments which are taken
# has changed (mostly -I, I think)  Only problem is that qmake gives all the 
# include paths and so on.
#
# Also this means that if you have both Qt3 and 4 installed, you need to use 
# the right qmake version to build a project! [end note]
#
